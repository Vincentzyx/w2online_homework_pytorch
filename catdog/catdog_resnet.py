...
class CatDogData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            imgArr = np.array(img.resize((128,128)))
            imgArr = imgArr / 255.0
...
if UseGPU:
    net = net.to(device)
train_data = CatDogData("", transform)
test_data = CatDogData("", transform, "test")
...
if os.path.exists("weights_resnet.pkl"):
    net.load_state_dict(torch.load('weights_resnet.pkl'))
...
for epoch in range(1000):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        if i % 10 == 0:
            ...
            stepCount = 0
        if i % 50 == 0:
            torch.save(net.state_dict(), 'weights_resnet.pkl')
            correctCount = 0
            totalCount = 0
